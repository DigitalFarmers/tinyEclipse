...
class Tenant(Base):
    ...
    sources = relationship("Source", back_populates="tenant", lazy="select")
    conversations = relationship("Conversation", back_populates="tenant", lazy="select")
    consents = relationship("Consent", back_populates="tenant", lazy="select")
    site_modules = relationship("SiteModule", back_populates="tenant", lazy="select")
    commands = relationship("CommandQueue", back_populates="tenant", lazy="select")
    push_subscriptions = relationship("PushSubscription", back_populates="tenant", lazy="select")
    alert_rules = relationship("AlertRule", back_populates="tenant", lazy="select")
    proactive_alerts = relationship("ProactiveAlert", back_populates="tenant", lazy="select")
